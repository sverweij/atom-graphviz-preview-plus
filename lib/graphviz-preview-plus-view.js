...
let renderer = null;
let errRenderer = null;
let svgToRaster = null;

let latestKnownEditorId = null;

module.exports = class GraphVizPreviewView extends ScrollView {
    static content() {
        this.div({
    ...
    handleEvents() {
        ...
        let changeHandler = () => {
            ...
            let pane = Boolean(atom.workspace.paneForItem(this))
    ...
    renderDot() {
        ...
        return this.getSource().then((source => {
            if (source !== null) {
                return this.renderDotText(source);
            }
            return null;
        }));
    }
    ...
    showError(error) {
        ...
        return this.getSource().then((source => {
            if (Boolean(source)) {
                return this.imageContainer.html(errRenderer.renderError(source, error.message));
            }
            return null;
        }));
    }
    ...
    saveAs(pOutputType) {
        ...
        let outputFilePath = atom.showSaveDialogSync(filePath);
        if (Boolean(outputFilePath)) {
            if ('png' === pOutputType) {
                if (svgToRaster === null) {
                    svgToRaster = require('./svg-to-raster');
                }
                svgToRaster.transform(this.svg, pResult => {
    ...
    adjustZoom(delta) {
        let zoomLevel = parseFloat(this.renderedSVG.css('zoom')) || 1;
