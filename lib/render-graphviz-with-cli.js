 ...
 */
export default function renderGraphVizWithCLI(pDot, pCallback, pOptions) {
  const lOptions = {
    format: "svg",
    engine: "dot",
    exec: "dot",
  ...
  dot.stdout.on("data", function handleStdOut(pData) {
    lData += pData;
  });
  dot.stderr.on("data", function handleStdError(pError) {
    lError = pError;
  });
  dot.on("error", function handleError(pError) {
    lError = pError;
  });
  dot.on("close", function handleClose(pCode) {
    //  0: okeleedokelee
